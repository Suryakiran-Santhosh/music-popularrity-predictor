import base64 #this is for token header as per api docs
import requests
import datetime
...
class SpotifyAPI:
    ...
    def get_resource(self, lookup_id, resource_type = "tracks"):
        base_endpoint = "https://api.spotify.com/v1"
        request_type = f"/{resource_type}/{lookup_id}"
        endpoint = f"{base_endpoint}{request_type}"
    ...
    def search(self, query = None, search_type = "track", limit = "1"):
        '''
        limit is number of results we want back
        '''
        if query == None:
